...
function MyContainer:OnContentsChanged()
    ...
    local width, height = self:LayoutButtons("grid", self.Settings.Columns, 5, 10, -45)
    width = width < 300 and 300 or width

    -- Update our size, reserve space for infobar at top
    self:SetSize(width + 20, height + 55)
end
...
function MyContainer:OnCreate(name, settings)
    ...
    space:SetPoint("TOPLEFT", infoFrame, 0, -5)
    space.bags = Simplicity:ParseBags(settings.Bags) -- Temporary until I find a better solution

    -- Money
    local money = self:SpawnPlugin("TagDisplay", "[money]", infoFrame)
    money:SetFontObject("NumberFontNormal")
    money:SetPoint("TOPRIGHT", infoFrame, -4, -2)

    -- Currencies
    local currency = self:SpawnPlugin("TagDisplay", "[currencies]", infoFrame)
    currency:SetFontObject("NumberFontNormal")
    currency:SetPoint("BOTTOMRIGHT", infoFrame, -8, 0)
